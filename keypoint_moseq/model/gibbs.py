...
def resample_model(data, *, states, params, hypparams, key, ar_only=False):
    key = jr.split(key)[1]
    
    params['betas'],params['pi'] = resample_hdp_transitions(
        key, **data, **states, **params, **hypparams['trans_hypparams'])
    
    params['Ab'],params['Q']= resample_ar_params(
        key, **data, **states, **params, **hypparams['ar_hypparams'])
    
    states['z'] = resample_stateseqs(
        key, **data, **states, **params)[0]
        
    if not ar_only:     
        
        params['sigmasq'] = resample_obs_variance(
            key, **data, **states, **params, **hypparams['obs_hypparams'])
        
        states['x'] = resample_latents(
            key, **data, **states, **params)
        
        states['h'] = resample_heading(
            key, **data, **states, **params)
        
        states['v'] = resample_location(
            key, **data, **states, **params, **hypparams['cen_hypparams'])
